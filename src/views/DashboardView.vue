  ...
  <div>
    <div class="flex items-start justify-between mb-6 gap-4">
      ...
      <button
        ...
        class="flex items-center gap-2 px-4 py-2 text-sm rounded-lg bg-emerald-600 hover:bg-emerald-500 font-medium transition-colors shrink-0"
      >
    ...
    <div v-if="!loading && urls.length > 0" class="grid grid-cols-3 gap-3 mb-6">
      ...
      <div class="bg-gray-900 border border-emerald-900/40 rounded-xl p-4">
        <p class="text-2xl font-semibold text-emerald-400">{{ summary.up }}</p>
        <p class="text-xs text-gray-500 mt-1">Activas</p>
      </div>
      <div class="bg-gray-900 rounded-xl p-4" :class="summary.down > 0 ? 'border border-red-900/40' : 'border border-gray-800'">
        <p class="text-2xl font-semibold" :class="summary.down > 0 ? 'text-red-400' : 'text-gray-600'">{{ summary.down }}</p>
    ...
    <div v-else class="space-y-2">
      <div
        ...
        class="group flex items-center gap-4 p-4 bg-gray-900 border border-gray-800 rounded-xl cursor-pointer hover:border-gray-600 hover:bg-gray-800/60 transition-all duration-150"
      >
        <StatusBadge :is-up="statuses[url.id]?.is_up ?? null" />

        <div class="flex-1 min-w-0">
          <p class="font-medium text-sm">{{ url.name }}</p>
          <p class="text-xs text-gray-500 truncate mt-0.5">{{ url.url }}</p>
        </div>

        <div class="text-right shrink-0 hidden sm:block">
          <p v-if="statuses[url.id]?.last_response_time_ms" class="text-sm font-medium text-gray-300">
        ...
        <svg
          class="w-4 h-4 text-gray-700 group-hover:text-gray-400 transition-colors shrink-0"
