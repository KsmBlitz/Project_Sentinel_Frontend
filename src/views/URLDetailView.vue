...
async function checkNow() {
  checking.value = true
  try {
    status.value = await urlsService.checkNow(props.id)
  } finally {
    ...
    <div v-else class="space-y-6">
      <div class="flex items-center gap-3 p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
        ...
        <button
          @click="checkNow"
          :disabled="checking"
          class="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-colors"
        >
          <svg class="w-3.5 h-3.5" :class="checking ? 'animate-spin' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
          {{ checking ? 'Chequeando…' : 'Verificar ahora' }}
        </button>
      </div>
